...
export default function MessagesPage() {
  ...
  const receiver = searchParams.get("user") || "";

  useEffect(() => {
    const token = localStorage.getItem("hamnavaz_token");
    if (!token) { router.replace(`/auth/login?next=${encodeURIComponent(`/messages${receiver ? `?user=${receiver}` : ""}`)}`); return; }
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      const created = await sendMessage(receiver, text.trim());
  ...
  return (
    <main className="container page-shell" dir="rtl">
      ...
      {!receiver && <div className="panel composer-hint">برای ارسال پیام، ابتدا از صفحه یک نوازنده وارد «ارسال پیام» شو.</div>}

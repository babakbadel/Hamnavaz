...
import HamnavazFinder from "./components/HamnavazFinder";
...
function LiveStage() {
  return <div className="stage" aria-label="صحنه زنده همنواز"><div className="stage-grid" /><div className="beam b1" /><div className="beam b2" /><div className="ring r1" /><div className="ring r2" /><div className="ring r3" /><div className="core"><Sparkles size={17} /><Music2 size={42} /><b>همنواز</b><small>PLAY TOGETHER</small></div><div className="instrument i1"><b>G</b><small>گیتار</small></div><div className="instrument i2"><b>V</b><small>ویولن</small></div><div className="instrument i3"><b>P</b><small>پیانو</small></div><div className="instrument i4"><b>D</b><small>درام</small></div><div className="eq" aria-hidden="true">{Array.from({ length: 18 }, (_, i) => <i key={i} style={{ animationDelay: `-${i * 70}ms` }} />)}</div><div className="live-pill"><span /> ۱٬۲۸۴ نوازنده آنلاین</div></div>;
}

export default function Home() {
  const [menuOpen, setMenuOpen] = useState(false);
  return <main dir="rtl" className="site">
    <header className="nav"><div className="nav-inner"><button className="menu-btn" onClick={() => setMenuOpen(v => !v)} aria-label="باز کردن منو">{menuOpen ? <X size={20} /> : <Menu size={20} />}</button><a className="brand" href="#" onClick={() => setMenuOpen(false)}><span className="brand-icon"><Music2 size={20} /></span><span><b>همنواز</b><small>HAMNAVAZ</small></span></a><nav className="desktop-nav"><a href="#find">همنواز</a><a href="#people">آدم‌ها</a><a href="#groups">گروه‌ها</a><a href="#teachers">اساتید</a><a href="#charity">خیریه</a></nav><a className="cta desktop" href="#find">شروع کن <ArrowLeft size={16} /></a></div>{menuOpen && <div className="mobile-menu"><a href="#find" onClick={() => setMenuOpen(false)}>همنواز</a><a href="#people" onClick={() => setMenuOpen(false)}>آدم‌ها</a><a href="#groups" onClick={() => setMenuOpen(false)}>گروه‌ها</a><a href="#teachers" onClick={() => setMenuOpen(false)}>اساتید</a><a href="#charity" onClick={() => setMenuOpen(false)}>خیریه</a></div>}</header>

    <section className="hero wrap"><div className="hero-copy"><div className="kicker"><i /> جامعه زنده موسیقی</div><h1>موسیقی را<br /><em>با هم</em> بسازیم.</h1><p>از اولین تمرین تا ساختن یک گروه و رسیدن به اجرای واقعی؛ آدم‌های هم‌مسیرت را پیدا کن.</p><form className="search" onSubmit={e => e.preventDefault()}><Search size={19} /><input aria-label="جستجوی نوازنده" placeholder="مثلاً گیتاریست مبتدی در اصفهان" /><button type="submit">پیدا کن <ArrowLeft size={16} /></button></form><div className="chips"><button>گیتاریست · اصفهان</button><button>استاد پیانو</button><button>گروه جَز · تهران</button></div><div className="hero-note"><span>01</span><div><b>از آدم درست شروع کن</b><small>یک ساز، یک شهر، یک مسیر.</small></div></div></div><LiveStage /></section>

    <section className="numbers wrap"><div><b>۱٬۲۸۴</b><span>نوازنده آنلاین</span></div><div><b>۸۶</b><span>گروه در حال شکل‌گیری</span></div><div><b>۲۴</b><span>اجرای پیش‌رو</span></div><div><b>۳۱۲</b><span>همکاری موفق</span></div></section>

    <section id="find" className="section wrap"><Head label="اولین قدم" title={<>آدم درست را <em>پیدا کن.</em></>} text="یک جست‌وجو؛ چند مسیر برای رسیدن به آدم موسیقی مورد نیازت." /><HamnavazFinder /><div className="path-grid"><article><b>01</b><Music2 /><h3>همنواز پیدا کن</h3><p>برای تمرین، پیشرفت یا یک گروه تازه.</p></article><article><b>02</b><Users /><h3>گروه بساز</h3><p>آدم‌هایی که با مسیر موسیقی تو هم‌جهت‌اند.</p></article><article><b>03</b><Heart /><h3>به یک اجرا برس</h3><p>از تمرین‌های کوچک تا یک صحنه واقعی.</p></article></div></section>
